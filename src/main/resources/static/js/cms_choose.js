...
Choose= {};
//模板选择弹框
Choose.template=function(name,url,callback) {
	layer.open({
		  title : name,
		  area:[350+'px',$(window).height() - 200+'px'],
	      fix: false, //不固定
	      maxmin: true,
	      content:  $.ajaxGetHtml(url),
...
//自定义弹框
Choose.dialog=function(name,content,callback) {
	layer.open({
		title : name,
		width : 600+'px',
		...
		success: function (layero, index) {
			var html = $(layero);
	  		html.find(".validform").each(function(){
	  			var fm=new Validform($(this),function(form){
	  				callback(form);
	  		 	});
	  		});
		},
...
//弹框加载页面
Choose.ajaxdialog=function(url,option,applayValue,doFocus,global){
	if (global === undefined) {
        global = true;
    }
	var layerOption={
	      ...
	      btn: ['确定','取消'],
		  yes: function(index, layero){
				if(option.callbacks && typeof option.callbacks.ok=="function") {
					applayValue();
				}
				if(doFocus){
					doFocus();
		  ...
		  cancel: function(index, layero){
				if(option.callbacks && typeof option.callbacks.cancel=="function") {
					option.callbacks.cancel();
				}
				if(doFocus){
					doFocus();
				}
		...
		$.extend(layerOption, option.settings);
...
//	$.extend(settings, options.settings);
	...
	f7Button.click(function() {
		...
		//$.param(params,true)
		Choose.ajaxdialog(url + "?" + $.param(params),options,applayValue);
//		dialogDiv.load(url, params, function() {
//			dialogDiv.dialog(settings);
//		});
	});
};
Cms.F7Single = function(url, name, names, focus, options,pid) {
	options = options || {};
	var idField;
	if(pid){
		idField = $("#" + pid)
...
//		}
		...
		Choose.ajaxdialog(url + "?" + $.param(params),options,applayValue,doFocus);
...
Cms.f7.template = function(name, options) {
	options = options || {};
	var url ="/cms/manage/choose/TemplateTree";
 ...
 * 附件选择
 */
Cms.f7.uploads = function(name, focus, options,pid) {
	options = options || {};
	var url = "/cms/manage/choose/chooseUploads";
	var settings = {title:"WebFile Select",width:700,height:450};
	options.settings = $.extend(settings, options.settings);
	var names = ["Name","Length"];
	var f7 = Cms.F7Single(url,name,names,focus,options,pid);
};
